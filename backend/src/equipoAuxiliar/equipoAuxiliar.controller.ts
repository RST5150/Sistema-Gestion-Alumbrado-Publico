...
async function findOne(req: Request, res: Response) {
  try {
    const equipoAuxiliar = await em.findOneOrFail(EquipoAux, {
      id: res.locals.id,
    });
    res.json({ data: equipoAuxiliar });
  } catch (err) {
...
async function add(req: Request, res: Response) {
  try {
    ...
    res
      .status(201)
      .json({ message: "Equipo Auxiliar creado", data: equipoAuxiliar });
  } catch (err) {
    handleOrmError(res, err);
  }
}
    ...
    switch (err.name) {
      case "NotFoundError":
        res
          .status(404)
          .json({
            message: `Equipo Auxiliar no encontrado para ese ID ${res.locals.id}`,
          });
        break;
      default:
        console.error("\n--- ORM ERROR ---");
