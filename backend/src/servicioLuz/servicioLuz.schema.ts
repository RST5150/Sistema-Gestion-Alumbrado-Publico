...
const fechaInstalacion = v.pipe(v.string(), v.isoDate(ERR_FECHA));
...
const columna = v.pipe(v.number(ERR_ID_COLUMNAS), v.integer(ERR_ID_COLUMNAS));
...
export const validarServicioLuzOpcional =
  v.safeParserAsync(servicioLuzOpcional);
